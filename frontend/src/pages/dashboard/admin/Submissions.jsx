            ...
            <button
              onClick={() => {
                const sorted = [...submissions].sort((a, b) => {
                  const titleA = a.ps_title || '';
                  const titleB = b.ps_title || '';
                  return titleA.localeCompare(titleB);
                });
                setSubmissions(sorted);
                toast.info("Sorted by Problem Statement");
              }}
              className="text-sm text-blue-600 hover:text-blue-800 font-medium"
            >
              Sort by Problem Statement
            </button>
          </div>
          ...
          {viewingSub && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {viewingSub.ppt_url && (
                  <div>
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Presentation</h4>
                    {viewingSub.ppt_url.toLowerCase().endsWith('.pdf') ? (
                      <iframe
                        src={viewingSub.ppt_url}
                      ...
                      <div className="bg-slate-50 p-4 rounded border border-slate-100 text-center">
                        ...
                        <a
                          href={viewingSub.ppt_url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 px-3 py-2 bg-blue-50 text-blue-700 rounded hover:bg-blue-100 text-sm font-medium"
                        >
