        ...
        <table className="w-full">
          ...
          <tbody>
            {collegeRegistrations.length === 0 ? (
              <tr><td className="p-8 text-center muted" colSpan={6}>No pending registrations</td></tr>
            ) : (
              collegeRegistrations.map(r=> (
                <tr key={r.id} className="border-t">
                  <td className="p-4">{r.collegeName}</td>
                  <td className="p-4">{r.spocName}</td>
                  ...
                  <td className="p-4">
                    <div className="flex gap-2">
                      ...
                      <button className="px-3 py-1 bg-red-50 text-red-700 rounded-md border" onClick={()=>rejectCollegeRegistration(r.id, 'Rejected by admin')}>Reject</button>
                    </div>
                  </td>
                </tr>
              ))
